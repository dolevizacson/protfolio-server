...
module.exports = class WorkingOnTaskService {
  async readAll() {
    const tasksList = await taskListTaslModel.find();
    if (!tasksList) {
  ...
  async readOne(id) {
    const task = await taskListTaslModel.findOne({ _id: id });
    if (!task) {
  ...
  async create(task) {
    return await taskListTaslModel.create(task);
  }

  async update(id, task) {
    const updatedTask = await taskListTaslModel.findOneAndUpdate(
      { _id: id },
      task,
      { new: true }
    );
  ...
  async toggle(id) {
    const toggledTask = await taskListTaslModel.findOneAndUpdate(
      { _id: id },
      {
        $bit: {
          isDone: { xor: 1 },
  ...
  async deleteOne(id) {
    const deletedTask = await taskListTaslModel.findOneAndDelete({
      _id: id,
    });
    if (!deletedTask) {
      throw new NotFoundInDatabase('Working on task not found in database');
